	...
	document.addEventListener('DOMContentLoaded', () => {
		...
		document.querySelectorAll('.btn-complete').forEach(button => {
			button.addEventListener('click', () => {
				...
				if (userAnswer.toLowerCase() === correctAnswer.toLowerCase()) {
					fadeBgmVolume(bgmElement, 0.1, 0);
					// 정답일 경우
				...
				if (stages.length === 0 && stagesToRetry.length === 0) {
					...
					setTimeout(() => {
						...
						// document.querySelector('.cart').classList.remove('moveCart');
						// documtwinkleent.querySelector('.game-stage.show')?.classList.remove('show');
		...
		document.addEventListener('click', function (e) {
			...
			if (e.target.closest('.btn-cmp') && e.target.closest('.act-stage01')) {
				...
				if (nowActStage && nowActStage.parentElement) {
