...
document.addEventListener('DOMContentLoaded', () => {
	...
	document.querySelectorAll('.choice-list li').forEach(choice => {
		choice.addEventListener('click', () => {
			...
			let btnComplete = document.querySelector(".stage-text.show").querySelector(".btn-complete");
	...
	function initializeDeleteButtons() {
		document.querySelectorAll('.btn-delete').forEach(button => {
			button.disabled = true; // 또는 setAttribute('disabled', 'disabled')
		});
	}
	...
	// 정답 확인
	const answers = {
		stage01: 'can i use the phone?',
		stage02: 'can i set the phone?',
		stage03: 'can i eat the phone?',
		stage04: 'can i move the phone?',
		stage05: 'can i handed the phone?',
		stage06: 'can i make the phone?',
	};
	...
	document.querySelectorAll('.btn-complete').forEach(button => {
		button.addEventListener('click', () => {
			...
			if (stages.length === 0 && stagesToRetry.length === 0) {
				...
				setTimeout(() => {
					// 게임 완료 처리
					// document.querySelector('.cart').classList.remove('moveCartOut');
					// document.querySelector('.cart').classList.remove('moveCart');
	...
	function goToNextStage() {
		...
		// console.log("다음 스테이지 확인:", nextStage ? "있음" : "없음", "다음 선택지 확인:", nextChoice ? "있음" : "없음");
		// console.log("남은 재시도 문제:", stagesToRetry.length);
		...
		function allCompleted() {
			...
			// 다음 단계로 진행 - act-stage01 표시
			const actStage = document.querySelector('.act-stage01');
			if (actStage) {
				actStage.classList.add('show');
			} else {
				console.error('act-stage01 요소를 찾을 수 없습니다. 바로 게임 완료로 진행합니다.');
				onGameComplete();
			}
		}
	}
	...
	document.addEventListener('click', function (e) {
		...
		if (e.target.closest('.btn-cmp') && e.target.closest('.act-stage01')) {
			...
			const nowActStage = e.target.closest('.act-stage01').querySelector(".step-wrp");
	...
	function onGameComplete() {
		...
		let message = '';
		if (jewelCount <= 2) {
			message = `아쉽다.<br> 다시도전해 볼까?`;
		} else if (jewelCount <= 4) {
			message = `고생했어! <br> 다음에 더 많은 <br> 보석을 캐보자`;
		} else {
			message = `고생했어!<br> 모든 보석을 다 캐다니<br> 대단한걸?`;
		}
		$('.ending-bubble').html(message);
	...
	$('.pick_jewel').on('click', function () {
		...
		const pickInterval = setInterval(() => {
			...
			// 보석 이미지 변경, 곡괭이 애니메이션, 체력 게이지 변화를 case 문에서 함께 관리
			switch (pickCount) {
				case 0:
					// 첫 번째 곡괭이질
					jewelImg.attr('src', `../img/game01/work-jewel${jewelNumber}_02.png`);
					// jewel.css({ width: '120px', left: pos.x - 50 + 'px' });

					// 체력 감소
					currentPickCount++;
					const remainingPercent1 = Math.max(0, ((totalPickCount - currentPickCount) / totalPickCount) * 100);
					$('.gauge-bar').css('width', `${remainingPercent1}%`);
					console.log(`[곡괭이질] 1회, 현재 체력: ${currentPickCount}/${totalPickCount} (${remainingPercent1}%)`);
				...
				case 1:
					...
					// jewel.css({ width: '100px', left: pos.x - 30 + 'px' });

					// 체력 감소
					currentPickCount++;
					const remainingPercent2 = Math.max(0, ((totalPickCount - currentPickCount) / totalPickCount) * 100);
					$('.gauge-bar').css('width', `${remainingPercent2}%`);
					console.log(`[곡괭이질] 2회, 현재 체력: ${currentPickCount}/${totalPickCount} (${remainingPercent2}%)`);
				...
				case 2:
					...
					// jewel.css({ width: '80px', left: pos.x + 'px' });
					jewel.addClass('picked');
					pickedJewels.push(className);

					// 체력 감소
					currentPickCount++;
					const remainingPercent3 = Math.max(0, ((totalPickCount - currentPickCount) / totalPickCount) * 100);
					$('.gauge-bar').css('width', `${remainingPercent3}%`);
					console.log(`[곡괭이질] 3회, 현재 체력: ${currentPickCount}/${totalPickCount} (${remainingPercent3}%)`);
					console.log(`[보석 획득] 보석 번호: ${jewelNumber}, 현재 획득한 보석: ${pickedJewels.length}개`);
	...
	function resetGame() {
		...
		document.querySelectorAll('.choice-list').forEach((choice, index) => {
			...
			choice.querySelectorAll('li').forEach(li => li.classList.remove('disabled'));
			choices.push(choice);
		});
