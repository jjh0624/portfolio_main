jQuery(function($){
	...
	function CANVAS(){
		...
		this.moveLine = function(startBtn, endBtn, color, width){
			var sx = offsetValue(startBtn.offset().left-this.parent.offset().left)+10;
			var sy = offsetValue(startBtn.offset().top-this.parent.offset().top)+10;
			var ex = offsetValue(endBtn.offset().left-this.parent.offset().left)+10;
			var ey = offsetValue(endBtn.offset().top-this.parent.offset().top)+10;

			this.ctx.lineWidth = width || 6;
			this.ctx.strokeStyle = color || '#085ec3';
	...
	function lineAnswerLoop(el, endParent, id){
		el.each(function(){
			...
			if(endBtn.length){
				Canvas[id].moveLine(startBtn, endBtn, "rgba(255,0,0,.7)", 12);
			}
		});
	}
		...
		if(Canvas[id].isCross){
			...
			$(el).find('[data-multi]').each(function(){
				...
				for (var i = 0; i < multiArr.length; i++) {
					...
					Canvas[id].moveLine($(this).children('button'),endBtn, "rgba(255,0,0,.7)", 12);
				}
			});
		}else{
			lineAnswerLoop(start, end, id);
